    ...
    if (productForm) {
        productForm.addEventListener('submit', async (e) => {
            ...
            document.querySelectorAll('.variant-row').forEach(row => {
                ...
                if (size && !isNaN(price)) {
                    variants.push({ size, price, quantity: qty });
                    totalVariantQty += qty;
                    hasVariants = true;
                }
            });
    ...
    if (productId) {
        ...
        if (product) {
            ...
            if (product.variants && Array.isArray(product.variants)) {
                product.variants.forEach(v => addVariantRow(v.size, v.price, v.quantity));
            }
...
function addVariantRow(size = '', price = '', quantity = '') {
    const container = document.getElementById('variants-container');
    if (!container) return;

    const row = document.createElement('div');
    row.className = 'variant-row';
    row.style = 'display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 10px; margin-bottom: 10px; align-items: center; background: white; padding: 12px; border-radius: 8px; border: 1px solid #ddd;';
    ...
    row.innerHTML = `
        <div>
            <label class="small" style="display:block; margin-bottom:5px;">المقاس</label>
            <input type="text" class="form-control v-size" value="${size}" placeholder="41" style="padding: 8px;">
        </div>
        <div>
